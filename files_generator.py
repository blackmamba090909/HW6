...
def generate_other_files(path):
    other_extensions = ('DAT', 'LOG', 'CFG', 'XML', 'JSON')
    with open(path / f"{get_random_filename()}.{choice(other_extensions).lower()}", "wb") as f:
        f.write(MESSAGE.encode())
...
def generate_random_files(path):
    for i in range(3, randint(5, 7)):
        function_list = [generate_text_files, generate_archive_files, generate_image, generate_other_files]
